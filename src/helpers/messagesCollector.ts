...
const messageRemover = (bot: Telegraf<Context>, iteration: number, message: any) => {
  setTimeout(() => {
    bot.telegram.deleteMessage(message.chatId, message.id).finally(() =>
      removeCollector(message.id)
    )
  }, 1000 * iteration * message.timestampInHours)
}

const messageCollectorHandler = async (bot: Telegraf<Context>) => {
  ...
  const filterMessagesToRemove = allGarbageMessages.filter(({ createdAt }) =>
